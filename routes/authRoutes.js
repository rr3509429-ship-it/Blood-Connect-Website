...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    if (passwordReset) {
      ...
      const validOtp = await bcrypt.compare(otp, passwordReset.otp);
      if (!validOtp) return res.status(400).json({ message: 'Invalid OTP.' });
      passwordReset.isVerified = true;
      await passwordReset.save();
      return res.json({ message: 'OTP verified. You may reset your password.', email });
    }
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    await PasswordReset.findOneAndUpdate(
      { email },
      { email, otp: hashedOtp, isVerified: false, expiresAt },
      { upsert: true, new: true, setDefaultsOnInsert: true }
    );

    const emailSent = await sendPasswordResetEmail(email, user.name || 'Friend', otp);
    res.json({
      message: emailSent
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    const passwordReset = await PasswordReset.findOne({ email });
    if (!passwordReset) {
      return res.status(400).json({ message: 'Valid OTP verification is required' });
    }
    if (!passwordReset.isVerified) {
      return res.status(400).json({ message: 'Please verify the OTP before resetting your password.' });
    }
    if (new Date() > passwordReset.expiresAt) {
      await passwordReset.deleteOne();
      return res.status(400).json({ message: 'OTP expired. Request a new one.' });
